...
func (d *DAL) SetPingAvailable(ctx context.Context, tgID int64, enabled bool) error {
	sql := `
		update fg_participant
		set ping_available = $2
		where tg_id = $1
	`

	_, err := d.db.Pool(ctx).Exec(ctx, sql, tgID, enabled)
	return err
}
